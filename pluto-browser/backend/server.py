...
async def on_NavigateToUrlEvent(self, event):
    ...
    try:
        ...
        if shell_target:
            ...
            if event.new_tab:
                self.logger.info(f"[patched_navigate] Creating new tab in Electron via IPC")
                expr = f"(async () => {{ await window.plutoAPI.createTab({json.dumps(event.url)}); return 'ok'; }})()"
            else:
                self.logger.info(f"[patched_navigate] Navigating active tab in Electron via IPC")
...
class WriteCodeToEditorParams(BaseModel):
    code: str = Field(..., description="The complete, fully completed code program (including all variables, print statements, and correct indentation) to write inside the Monaco web IDE editor.")
...
async def write_code_to_editor(params: WriteCodeToEditorParams, browser_session: Browser) -> ActionResult:
    try:
        ...
        if res in ["monaco_success", "ace_success"]:
            ...
            return ActionResult(is_done=False, extracted_content=f"Successfully injected code into {res} editor and triggered state synchronization.")
        ...
        raise Exception("Could not find any editor element to focus (tried .ace_text-input, .ace_editor, .monaco-editor, textarea).")
    except Exception as e:
...
async def login_to_codechef(params: LoginParams, browser_session: Browser) -> ActionResult:
    try:
        ...
        return ActionResult(is_done=False, extracted_content="Logged in to CodeChef successfully and navigated back to the DAA course page.")
    except Exception as e:
    ...
    if ls_address and csrf_token:
        try:
            ...
            r = httpx.post(
                ...
                headers=headers, json={}, timeout=1.5,
            )
            if r.status_code == 200:
                data = r.json()
...
def resolve_llm(model_spec: str | None):
    ...
    for key, factory in [
        ("ANTIGRAVITY_LS_ADDRESS", lambda: __import__("browser_use.llm.antigravity.chat", fromlist=["ChatAntigravity"]).ChatAntigravity(
        ...
        ("OPENROUTER_API_KEY", lambda: ChatOpenAI(
            model="google/gemini-2.5-flash",
            base_url="https://openrouter.ai/api/v1",
            api_key=os.getenv("OPENROUTER_API_KEY"),
            max_completion_tokens=int(os.getenv("OPENROUTER_MAX_TOKENS", "2048")),
        )),
        ("ANTHROPIC_API_KEY", lambda: __import__("browser_use", fromlist=["ChatAnthropic"]).ChatAnthropic(model="claude-sonnet-4-0", api_key=os.getenv("ANTHROPIC_API_KEY"))),
    ]:
        val = os.getenv(key, "")
        if val and not val.startswith("your_"):
            return factory()
